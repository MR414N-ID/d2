...
module.exports = kurr = async (kurr, mek) => {
    try {
        ...
        const katalog = (teks) => {
            res = kurr.prepareMessageFromContent(from, { "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Lanoxゑ v.3.0_*", "thumbnail": ofrply, "surface": 'CATALOG' } }, { quoted: ftrol })
            kurr.relayWAMessage(res)
        }
        const grupinv = (teks) => {
            grup = kurr.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `Lanoxゑ v.3.0`, "footerText": "*_© Lanoxゑ v.3.0_*", "jpegThumbnail": ofrply, "caption": teks } }, { quoted: finv })
        ...
        if (!mek.key.remoteJid.endsWith('@g.us') && offline) {
            if (!mek.key.fromMe) {
                ...
                sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© Lanoxゑ v.3.0_*", [{ buttonId: 'simi', buttonText: { displayText: 'Oke' }, type: 1 }], { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true } })
            }
        }
        if (mek.key.remoteJid.endsWith('@g.us') && offline) {
            if (!mek.key.fromMe) {
        ...
        const reply = (teks) => {
            kurr.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply: { title: `${jmn} - ${week}  - ${calender}`, body: "Lanoxゑ v.3.0", previewType: "PHOTO", thumbnail: ofrply, sourceUrl: `https://wa.me/p/6349482305092740/6281220670449` } } })
        }
        // GAME 
        ...
        if (budy.includes('Assalamualaikum')) {
            cpref = `Hay Saya Lanox Botz Whatsapp Ketik .menu Untuk Mengakses`
            reply(cpref)
        }
        if (budy.includes('assalamualaikum')) {
        ...
        //runtime berjalan
        runi = process.uptime()
        kurr.setStatus(`Saya Adalah Bot Domain`).catch((_) => _);
        ...
        switch (command) {
            //Batas OM//
            case "menu":
              if(from !="120363042628706503@g.us") return reply(`AKSES BOT INI? CHAT https://wa.me/6281220670449`)
            ini_txt = `BOT DOMAIN
CARA PAKAI BOT DOMAIN KETIK
#DOMAIN HOSTNAME|IP
CONTOH:
#DOMAIN KONTOLODONUS|20.212.35.190`
reply(ini_txt)
break
 case "domain":
   if(from !="120363042628706503@g.us") return reply(`AKSES BOT INI? CHAT https://wa.me/6281220670449`)
